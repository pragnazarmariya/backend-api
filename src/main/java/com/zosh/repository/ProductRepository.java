...
//	@Query("SELECT p From Product p Where p.subCategory=:category")
	...
	@Query("SELECT p From Product p where LOWER(p.name) Like %:query% OR LOWER(p.description) Like %:query% OR LOWER(p.brand) LIKE %:query% OR LOWER(p.category.name) LIKE %:query%")
